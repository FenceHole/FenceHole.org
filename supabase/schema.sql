...
create or replace function public.is_team_member() returns boolean language sql security definer stable set search_path = public, pg_temp as $$ select exists (select 1 from public.profiles where id = auth.uid() and role = 'team') $$;
create policy "Team sees all" on profiles for select using (public.is_team_member() or auth.uid()=id);
...
create policy "Team can view" on access_requests for select using (public.is_team_member());
...
create policy "Team full access contacts" on contacts for all using (public.is_team_member());
...
create policy "Team full access deals" on deals for all using (public.is_team_member());
...
create policy "Team full access notes" on notes for all using (public.is_team_member());
...
create policy "Team full access tasks" on tasks for all using (public.is_team_member());
...
create policy "Team full access content" on content_ideas for all using (public.is_team_member());
...
create policy "Team sees all projects" on client_projects for all using (public.is_team_member());
...
create policy "Team sees all messages" on client_messages for all using (public.is_team_member());
